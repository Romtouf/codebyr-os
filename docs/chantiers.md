...
| 🟡 | **Profils AppArmor — le filtre réseau est confiné (1.12.1)** | `codebyr-net-proxy`, le seul programme qui parle au réseau pour un Espace peut-être compromis, tourne sous un profil strict : socket héritée, résolution de noms, connexions sortantes, journal des refus, et rien d'autre. **Restent** `codebyr-space` et l'extension GNOME, tous deux non confinés | M |
