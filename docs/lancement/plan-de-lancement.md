...
1. **Captures d'écran** → voir [captures.md](captures.md). Sans elles, ne pas
   lancer : un README d'OS sans image ne convertit personne.
2. **Vidéo démo 30 s** → voir [video-demo.md](video-demo.md). Optionnelle mais
   c'est l'atout n°1 (le Jetable est *démontrable*, chose rare pour une distro).
3. README mis à jour avec la galerie de captures.
4. Vérifier que la Release télécharge bien (tester le lien depuis un autre
   appareil) et que `sha256sum -c SHA256SUMS` passe.
5. **Répondre présent le jour J** : bloquer la journée. Les 6 premières heures
...
| 9 h 30 | **Mastodon + X** | [posts/mastodon-x.md](posts/mastodon-x.md) avec la vidéo |
